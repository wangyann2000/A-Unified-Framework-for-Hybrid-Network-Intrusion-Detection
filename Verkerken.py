...
class ConfusionMatrix(object):
    ...
    def plot(self):
        ...
        result_dir = f'./result/{opt.dataset}/TNSM2023/hybrid/{opt.split}/'
...
def manifold_visualization(score):
    ...
    result_dir = f'./result/{opt.dataset}/TNSM2023/discrimination/{opt.split}/'
    os.makedirs(result_dir, exist_ok=True)
...
def histogram_plot(pred_score, stage):
    ...
    # detection
    if stage == 1:
        result_dir = f'./result/{opt.dataset}/TNSM2023/detection/{opt.split}/'
...
opt = load_args("args/TNSM2023/cicids_args.json")
...
with torch.no_grad():
    ...
    # Additional step: reclassify samples predicted as unseen classes with low detector scores as benign
    # Find samples in preds_all that are predicted as unseen classes
    preds_all_tensor = torch.from_numpy(preds_all).to(device)
    unseen_classes_device = dataset.unseen_classes.to(device)

    # Find indices where preds_all contains unseen class labels
    predicted_as_unseen_mask = torch.isin(preds_all_tensor, unseen_classes_device)
    predicted_as_unseen_indices = torch.where(predicted_as_unseen_mask)[0]

    if len(predicted_as_unseen_indices) > 0:
        # Get detector_score for samples predicted as unseen
        detector_scores_tensor = torch.tensor(detector_score).to(device)
        predicted_unseen_detector_scores = detector_scores_tensor[predicted_as_unseen_indices]

        # Among predicted_as_unseen_indices, find those with true labels in unseen_classes
        true_labels_all = dataset.test_label.to(device)
        predicted_unseen_true_labels = true_labels_all[predicted_as_unseen_indices]
        true_unseen_mask_in_predicted = torch.isin(predicted_unseen_true_labels, unseen_classes_device)

        if true_unseen_mask_in_predicted.sum() > 0:
            # Get detector_score for these truly unseen samples among the predicted ones
            true_unseen_detector_scores_in_predicted = predicted_unseen_detector_scores[true_unseen_mask_in_predicted]

            # Calculate the threshold for TPR≥threshold
            sorted_scores, _ = torch.sort(true_unseen_detector_scores_in_predicted)
            tpr_index = max(0, int(len(sorted_scores) * (1 - opt.threshold)) - 1)
            detector_threshold = sorted_scores[tpr_index]

            print(f"Total number of samples predicted as unseen malicious traffic: {len(predicted_as_unseen_indices)}")
            print(f"Among them, true unseen malicious samples: {true_unseen_mask_in_predicted.sum().item()}")

            # Step 4: Apply threshold to ALL predicted_as_unseen_indices
            low_detector_score_mask = predicted_unseen_detector_scores < detector_threshold
            reclassify_to_benign_indices = predicted_as_unseen_indices[low_detector_score_mask]

            print(f"Number of samples to be reclassified as benign traffic: {len(reclassify_to_benign_indices)}")

            # Reclassify these samples as benign (label 0) using vectorized operation
            if len(reclassify_to_benign_indices) > 0:
                preds_all[reclassify_to_benign_indices.cpu().numpy()] = 0
